...
check_dependencies() {
    local deps=("qemu-system-x86_64" "wget" "cloud-localds" "qemu-img" "openssl" "ss" "pgrep" "pkill")
    local missing_deps=()
    ...
    if [ ${#missing_deps[@]} -ne 0 ]; then
        ...
        print_status "INFO" "On Ubuntu/Debian, try: sudo apt install qemu-system cloud-image-utils wget openssl iproute2 procps"
        exit 1
    fi
}
...
load_vm_config() {
    ...
    if [[ -f "$config_file" ]]; then
        ...
        if [[ -z "${VM_NAME:-}" ]]; then
            print_status "ERROR" "Invalid configuration file: VM_NAME missing"
            return 1
        fi
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            ...
            if [[ -n "$qemu_pid" ]]; then
                ...
                if [[ -f "$IMG_FILE" ]]; then
                    du -h "$IMG_FILE"
                    echo "Disk format: $(qemu-img info "$IMG_FILE" | grep -oP 'format: \K\w+')"
                else
